...
def npy_to_mesh(*, yml_input_file: Path) -> int:
    ...
    * Runs Sculpt via the command: sculpt -i <file>.i
    ...
    print(f"{ATMESH_PROMPT} Processing file: {fin}")
    ...
    print(f"{ATMESH_PROMPT} Success: database created from file: {fin}")
    print(yml_db)
    ...
    si += "  stair = 1\n"  # hardcode stair step to on
    si += f"  mesh_void = {recipe.mesh_void}\n"
    si += f"  input_spn = {path_spn}\n"
    # si += f"  exodus_file = {yml_input_file.parent.joinpath(yml_input_file.stem).expanduser()}\n"
    si += f"  exodus_file = {fin.parent.joinpath(fin.stem)}\n"
    si += f"  spn_xyz_order = {recipe.spn_xyz_order}\n"
    si += "END SCULPT\n"

    path_sculpt_i = (
        yml_input_file.resolve()
        .parent.joinpath(yml_input_file.stem + ".i")
        .expanduser()
    )
